...
{	
	...
	LCD_OpenWindow(usCOLUMN, usPAGE, 1, 1); //set
	LCD_FillColor(1, usColor);  //fill
...
{	
	...
	for (angle = 0; angle < 6.2832f; angle += 0.001f){   //2pi
	    x = (uint16_t)(usC + LR * cos(angle));
	    y = (uint16_t)(usP + SR * sin(angle));
...
{
    uint16_t row, col;
    uint16_t byte_idx, bit;

    for (row = 0; row < 24; row++)
    {
        for (col = 0; col < 24; col++)
        {
            byte_idx = row * 3 + col / 8;   //  3 bytes per row pick 012
            bit      = col % 8;  //0-7
            if (bitmap[byte_idx] & (0x80 >> bit))  // MSB scan left to right if true drawdot
                LCD_DrawDot(usC + col, usP + row , BLACK);
        }
    }
}
